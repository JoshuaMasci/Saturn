...
ShapeHandle create_mesh_shape(const float positions[][3], size_t position_count, const uint32_t *indices, size_t indices_count) {
...
bool raycast(PhysicsWorld *ptr, ObjectLayer object_layer_pattern, const float origin[3], const float direction[3]) {
  auto physics_world = (PhysicsWorld *)ptr;
  ...
  bool had_hit = physics_world->physics_system->GetNarrowPhaseQuery().CastRay(
	  JPH::RRayCast(ray), hit, JPH::BroadPhaseLayerFilter(), AnyMatchObjectLayerFilter(object_layer_pattern), JPH::BodyFilter());
