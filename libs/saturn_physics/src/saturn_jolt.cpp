...
bool cast_ray(PhysicsWorld *ptr, ObjectLayer object_layer_pattern, const float origin[3], const float direction[3]) {
	...
	bool had_hit = physics_world->physics_system->GetNarrowPhaseQuery().CastRay(JPH::RRayCast(ray), hit, JPH::BroadPhaseLayerFilter(), AnyMatchObjectLayerFilter(object_layer_pattern), JPH::BodyFilter());
	return had_hit;
}
