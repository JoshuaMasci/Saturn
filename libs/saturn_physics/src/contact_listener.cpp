...
void MyContactListener::OnContactAdded(const JPH::Body &inBody1, const JPH::Body &inBody2, const JPH::ContactManifold &inManifold, JPH::ContactSettings &ioSettings) {
  ...
  if (this->physics_world->volume_bodies.find(inBody1ID) !=
	  this->physics_world->volume_bodies.end()) {
	this->physics_world->volume_bodies[inBody1ID].contact_list.add(inBody2ID);
  }

  if (this->physics_world->volume_bodies.find(inBody2ID) !=
	  this->physics_world->volume_bodies.end()) {
...
void MyContactListener::OnContactRemoved(const JPH::SubShapeIDPair &inSubShapePair) {
  ...
  if (this->physics_world->volume_bodies.find(inBody1ID) !=
	  this->physics_world->volume_bodies.end()) {
	this->physics_world->volume_bodies[inBody1ID].contact_list.remove(
		inBody2ID);
  }

  if (this->physics_world->volume_bodies.find(inBody2ID) !=
	  this->physics_world->volume_bodies.end()) {
	this->physics_world->volume_bodies[inBody2ID].contact_list.remove(
		inBody1ID);
  }
}
