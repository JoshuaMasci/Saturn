...
Character::Character(PhysicsWorld *physics_world, JPH::RefConst<JPH::Shape> shape, const JPH::RVec3 position, const JPH::Quat rotation) {
...
void Character::update(PhysicsWorld *physics_world, float delta_time) {
	...
	update_settings.mStickToFloorStepDown =
		this->character->GetRotation().RotateAxisY() * -0.5f;
	update_settings.mWalkStairsStepUp =
		this->character->GetRotation().RotateAxisY() * 0.4f;
	this->character->ExtendedUpdate(
		delta_time, this->gravity_velocity, update_settings, JPH::BroadPhaseLayerFilter(), JPH::ObjectLayerFilter(), JPH::BodyFilter(), JPH::ShapeFilter(), physics_world->temp_allocator);
}

void Character::OnContactAdded(const JPH::CharacterVirtual *inCharacter, const JPH::BodyID &inBodyID2, const JPH::SubShapeID &inSubShapeID2, JPH::RVec3Arg inContactPosition, JPH::Vec3Arg inContactNormal, JPH::CharacterContactSettings &ioSettings) {
