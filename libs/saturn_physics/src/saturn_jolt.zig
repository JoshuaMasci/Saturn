...
const c = @cImport({ @cInclude("saturn_jolt.h"); });
...
pub const Shape = struct {
  ...
  pub fn init_mesh(positions : [][3] f32, indices : [] u32) Self {
    return.{
      .handle = c.create_mesh_shape(
          @alignCast(@ptrCast(positions.ptr)), positions.len,
          @alignCast(@ptrCast(indices.ptr)), indices.len),
    };
  }

  pub fn deinit(self : Self) void { c.destroy_shape(self.handle); }
};
...
pub const World = struct {
  ...
  pub const Settings = struct {
    max_bodies : u32 = 1024, num_body_mutexes : u32 = 0,
  ...
  pub fn deinit(self : *Self) void { c.destroy_physics_world(self.ptr); }
  ...
  pub fn add_body(self
                  : *Self, body_settings
                  : *const BodySettings) BodyHandle {
    const c_body_settings : c.BodySettings =.{
      .shape = body_settings.shape.handle, .position = body_settings.position,
  ...
  pub fn set_body_linear_velocity(self
                                  : *Self, handle
                                  : BodyHandle, velocity
                                  : [3] f32) void {
  ...
  pub fn add_character(self
                       : *Self, shape
                       : Shape, transform
                       : *const Transform) CharacterHandle {
  ...
  pub fn get_character_ground_velocity(self
                                       : *Self, handle
                                       : CharacterHandle)[3] f32 {
    var velocity : [3] f32 =.{0.0, 0.0, 0.0};
  ...
  pub fn get_character_ground_state(self
                                    : *Self, handle
                                    : CharacterHandle) GroundState {
    return @enumFromInt(c.get_character_ground_state(self.ptr, handle));
  }

  pub fn raycast(self
                 : *Self, object_layer_pattern
                 : u16, origin
                 : [3] f32, direction
                 : [3] f32) bool {
    return c.raycast(self.ptr, object_layer_pattern, @ptrCast(&origin[0]),
                     @ptrCast(&direction[0]));
  }
};
...
fn zjoltAlignedAlloc(size
  ...
  const ptr = mem_allocator.
      ?.rawAlloc(size, std.math.log2_int(u29, @as(u29, @intCast(alignment))),
                 @returnAddress(), );
  if (ptr == null)
  ...
  mem_allocations.?.put(
                       @intFromPtr(ptr), .{
                         ...
                         .alignment = @as(u16, @intCast(alignment))
                       }, ) catch @panic("zjolt: out of memory");

  return ptr;
}

fn zjoltReallocate(maybe_ptr
                   :
                   ? *anyopaque, old_size : usize, new_size
                   : usize) callconv(.C)
    ? *anyopaque {
  if (maybe_ptr)
    | old_ptr | {
