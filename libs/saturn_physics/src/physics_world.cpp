    ...
    if (current_index != -1) {
        ...
        if (this->contact_count[current_index] <= 0) {
            ...
            if (current_index != last_index) {
                ...
                JPH::swap(this->contact_count[current_index],
                          this->contact_count[last_index]);
            }
            this->ids.pop_back();
...
size_t ContactList::size() { return this->ids.size(); }
...
PhysicsWorld::add_character(JPH::RefConst<JPH::Shape> shape, const JPH::RVec3 position, const JPH::Quat rotation,
                            uint64_t user_data,
                            JPH::RefConst<JPH::Shape> inner_shape, JPH::ObjectLayer inner_object_layer) {
    auto new_character = alloc_t<Character>();
    ::new(new_character) Character(this, std::move(shape), position, rotation, user_data, std::move(inner_shape),
                                   inner_object_layer);
