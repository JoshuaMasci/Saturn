...
transfer_list: std.ArrayList(BufferTransfer) = .empty,
...
pub fn writeBuffer(self: *Self, dst: Backend.BufferHandle, offset: usize, data: []const u8) WriteBufferError!void {
    ...
    self.transfer_list.append(self.allocator, .{
...
pub fn createRenderPass(
    ...
    if (self.transfer_list.items.len == 0) {
        return null;
    }

    defer {
        self.transfer_list.clearRetainingCapacity();
        self.staging_offset = 0;
    }

    const transfer_list = try temp_allocator.dupe(BufferTransfer, self.transfer_list.items);
...
const BuildData = struct {
    src_buffer: Buffer,
    transfer_list: []const BufferTransfer,
};
...
fn buildCommandBuffer(build_data: ?*anyopaque, backend: *Backend, resources: rg.Resources, command_buffer: vk.CommandBufferProxy, raster_pass_extent: ?vk.Extent2D) void {
    ...
    for (data.transfer_list) |transfer| {
