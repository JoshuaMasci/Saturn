...
pub fn init(gpa: std.mem.Allocator, device: saturn.DeviceInterface, registry: *const AssetRegistry, formats: RenderTargetState) !Self {
    ...
    const indirect: IndirectScenePass = try .init(gpa, device, registry, formats);
...
const IndirectScenePass = struct {
    ...
    pub fn init(gpa: std.mem.Allocator, device: saturn.DeviceInterface, registry: *const AssetRegistry, formats: RenderTargetState) !IndirectScenePass {
        ...
        const vertex_shader = try utils.loadShader(gpa, device, registry, .fromRepoPath("engine", "shaders/glsl/draw_indirect.vert.asset"));
        defer device.destroyShaderModule(vertex_shader);

        const opaque_frag_shader = try utils.loadShader(gpa, device, registry, .fromRepoPath("engine", "shaders/glsl/opaque_indirect.frag.asset"));
        defer device.destroyShaderModule(opaque_frag_shader);

        const alpha_mask_frag_shader = try utils.loadShader(gpa, device, registry, .fromRepoPath("engine", "shaders/glsl/alpha_mask_indirect.frag.asset"));
        defer device.destroyShaderModule(alpha_mask_frag_shader);
        ...
        return .{
            .build_pipeline = indirect_build_pipeline,
            .opaque_pipeline = opaque_pipeline,
            .alpha_mask_pipeline = alpha_mask_pipeline,
            .alpha_blend_pipeline = alpha_blend_pipeline,
        };
    ...
    pub fn deinit(self: *const IndirectScenePass, device: saturn.DeviceInterface) void {
        device.destroyComputePipeline(self.build_pipeline);
    ...
    pub fn addPasses(
        ...
        tpa: std.mem.Allocator,
        color_target: saturn.RGTextureHandle,
        depth_target: saturn.RGTextureHandle,
        render_graph: *saturn.RenderGraph,
        scene: *const Scene,
        camera: *const Camera,
        settings: *Settings,
        ...
        const indirect_draw_cmds_buffer = try render_graph.createTransientBuffer(.{
            .size = @sizeOf(u32) * 16,
            .usage = .{ .indirect = true, .device_address = true, .transfer_dst = true },
            .memory = .gpu_only,
        });
        ...
        if (opaque_primitive_count != 0) {
            const opaque_bucket_data: RenderBucketData = .{
                ...
                .indirect_draw_cmds_buffer = try render_graph.createTransientBuffer(.{
                    .size = @sizeOf(DrawIndexedIndirectCommandInfo) * opaque_primitive_count,
                    .usage = .{ .indirect = true, .device_address = true },
                    .memory = .gpu_only,
                }),
                ...
                .sort_pipeline = null,
                .draw_pipeline = self.opaque_pipeline,
            };
            ...
            try addDrawPass(
                "Draw Opaque Indirect",
                render_graph,
                .{
                    .color_attachments = &.{
                        .{ .texture = color_target, .clear = .{ 0.0, 0.0, 0.0, 1.0 } },
                    },
                    .depth_attachment = .{ .texture = depth_target, .clear = 1.0 },
                },
                scene_data,
                opaque_bucket_data,
            );
    ...
    fn addDrawPass(
        name: []const u8,
        render_graph: *saturn.RenderGraph,
        render_target: saturn.RGRenderTarget,
        scene_data: SceneData,
        bucket_data: RenderBucketData,
    ) saturn.Error!void {
        const ctx = try render_graph.dupe(IndirectBucketData, .{ .scene = scene_data, .bucket = bucket_data });
        const draw_pass = try render_graph.addGraphicsPass(name, render_target, ctx, drawPassCallback);

        try render_graph.addBufferUsage(draw_pass, scene_data.vertex_buffer, .vertex_read);
        try render_graph.addBufferUsage(draw_pass, scene_data.index_buffer, .index_read);

        try render_graph.addBufferUsage(draw_pass, scene_data.instance_buffer, .compute_storage_read);
        try render_graph.addBufferUsage(draw_pass, scene_data.texture_info_buffer, .compute_storage_read);
        try render_graph.addBufferUsage(draw_pass, bucket_data.material_instance_buffer, .compute_storage_read);

        try render_graph.addBufferUsage(draw_pass, bucket_data.indirect_draw_counts_buffer, .indirect_read);
        try render_graph.addBufferUsage(draw_pass, bucket_data.indirect_draw_cmds_buffer, .indirect_read);
    }
    ...
    const DrawPushData = extern struct {
        view_projection_matrix: zm.Mat,
    ...
    fn drawPassCallback(ctx: ?*anyopaque, cmd: saturn.GraphicsCommandEncoder, target_resolution: [2]u32) void {
        const data: *IndirectBucketData = @ptrCast(@alignCast(ctx.?));
        ...
        const view_matrix = data.scene.camera.transform.getViewMatrix();
        var projection_matrix = data.scene.camera.camera.getProjectionMatrix(aspect_ratio);
        projection_matrix[1][1] *= -1.0; //TODO: only do this for vulkan
        const view_projection_matrix = zm.mul(view_matrix, projection_matrix);

        cmd.setPipeline(data.bucket.draw_pipeline);

        cmd.setVertexBuffer(0, .from(data.scene.vertex_buffer), 0);
        cmd.setIndexBuffer(.from(data.scene.index_buffer), .u32, 0);

        cmd.pushConstants(DrawPushData, .{
            .view_projection_matrix = view_projection_matrix,

            .scene_instances_ptr = cmd.getBufferInfo(.from(data.scene.instance_buffer)).?.device_address.?,
            .indirect_command_infos_ptr = cmd.getBufferInfo(.from(data.bucket.indirect_draw_cmds_buffer)).?.device_address.?,

            .texture_info_binding = cmd.getBufferInfo(.from(data.scene.texture_info_buffer)).?.storage.?,
            .material_instance_binding = cmd.getBufferInfo(.from(data.bucket.material_instance_buffer)).?.storage.?,
        });

        cmd.drawIndexedIndirectCount(
            .from(data.bucket.indirect_draw_cmds_buffer),
            0,
            .from(data.bucket.indirect_draw_counts_buffer),
            data.bucket.indirect_draw_count_index * @sizeOf(u32),
            data.bucket.primitive_count,
            @sizeOf(DrawIndexedIndirectCommandInfo),
        );
    }
};
