...
vec4 transformSphere(mat4 model_matix, vec4 sphere_pos_radius) {
    const vec4 new_pos = (model_matix * vec4(sphere_pos_radius.xyz, 1.0));
    ...
    return vec4(new_pos.xyz, new_radius);
}
